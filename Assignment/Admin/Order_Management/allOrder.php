...
<nav>
    ...
    <main style="padding: 30px;">
        <div style="text-align: center; margin-bottom:20px; margin-top:20px"><h1>All Orders</h1></div>
...
<?php
    ...
    $query = 'SELECT order.id, order.transaction_id , user.name as user_name, transaction.payment_info, product.name as product_name,order.quantity, (product.price * order.quantity) as total, order.status from `order`, `transaction`, `user`, `product` where order.transaction_id = transaction.id and transaction.user_id = user.id and order.product_id = product.id '.$additionalQuery.' limit '.$firstIndex.', '.$limit;
    ...
    while($row = mysqli_fetch_array($result))
    {
        ...
        {
?>
        <td><a href="allOrder.php?acpt=<?php echo $row['id'];?>&search=<?php echo $_GET['search']; ?>" onclick="return confirm_alert(this);">Accept Order</a></td>
<?php
        } else if($row['status'] == 1)
        {
?>
        <td><a href="allOrder.php?finish=<?php echo $row['id'];?>&search=<?php echo $_GET['search']; ?>" onclick="return confirm_alert(this);">Finish Order</a></td>
...
<?php
    $GetPageNum = $conn->query('select count(id) as total from `order` where order.status = 2 '. $additionalQuery);
    if(!$GetPageNum) {
        echo "Error select: " . $conn->error . "<br/>";
    }
...
<?php 
    if(isset($_GET['acpt']))
    {
        ...
        {
            header("Location:../Order_Management/allOrder.php?search=". $_GET['search'] ."");
            die;
        }
    }
?>
...
<?php 
    ...
    {
        $orderID = $_GET['finish'];
        $UpdateOrder = 'update `order` set status = 2 where order.id='.$orderID.'';
        $result = $conn->query($UpdateOrder);
        if (!$result)
        {
            echo "Edit failed: $query<br/>" . $conn->error . "<br/><br/>";
