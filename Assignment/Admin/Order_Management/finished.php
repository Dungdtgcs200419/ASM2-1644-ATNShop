...
<nav>
    ...
    <main style="padding: 30px;">
        <div style="text-align: center; margin-bottom:20px; margin-top:20px"><h1>Finished Orders</h1></div>
...
<?php
    ...
    $query = 'SELECT order.id, order.transaction_id , user.name as user_name, transaction.payment_info, product.name as product_name,order.quantity, (product.price * order.quantity) as total, order.status from `order`, `transaction`, `user`, `product` where order.transaction_id = transaction.id and transaction.user_id = user.id and order.product_id = product.id and order.status = 2 '.$additionalQuery.' limit '.$firstIndex.', '.$limit;
