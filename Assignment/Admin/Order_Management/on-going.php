...
<head>
    ...
    <script>
        function confirm_alert(node) {
            return confirm("Do you want to finish this order?");
        }
    </script>
</head>
<body>
<nav>
    ...
    <main style="padding: 30px;">
        <div style="text-align: center; margin-bottom:20px; margin-top:20px"><h1>On Going Orders</h1></div>
...
<?php
    ...
    $query = 'SELECT order.id, order.transaction_id , user.name as user_name, transaction.payment_info, product.name as product_name,order.quantity, (product.price * order.quantity) as total, order.status from `order`, `transaction`, `user`, `product` where order.transaction_id = transaction.id and transaction.user_id = user.id and order.product_id = product.id and order.status = 1 '.$additionalQuery.' limit '.$firstIndex.', '.$limit;
...
?>
        ...
        <td><a href="on-going.php?id=<?php echo $row['id'];?>" onclick="return confirm_alert(this);">Finish Order</a></td>
...
<?php
    $GetPageNum = $conn->query('select count(id) as total from `order` where order.status = 1 '. $additionalQuery);
    if(!$GetPageNum) {
        echo "Error select: " . $conn->error . "<br/>";
    }
...
<?php 
    if(isset($_GET['id']))
    {
        ...
        {
            header("Location:../Order_Management/on-going.php");
            die;
        }
    }
?>