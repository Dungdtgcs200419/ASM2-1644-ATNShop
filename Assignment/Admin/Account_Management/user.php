...
<head>
    ...
    <title>Account Management (user)</title>
...
<nav>
    ...
    <main style="padding: 30px;">
        <div style="text-align: center; margin-bottom:20px; margin-top:20px"><h1>Account Management (user)</h1></div>
        <div class="row">
            <div class="col-lg-6"></div>
            <div class="col-lg-6">
                <form method="get">
                    <div class="form-group">
                        <input type="text" class="form-control" placeholder="Search" id="search" name="search" style="width: 500px;  float:right; margin-top: 10px; margin-bottom:20px;">
                    </div>
                 </form>
            </div>
        </div>
...
<?php
    ...
    $query = 'Select * from user where 1 '.$additionalQuery.' limit '.$firstIndex.', '.$limit;
...
<?php
    $GetPageNum = $conn->query('select count(id) as total from user where 1 '. $additionalQuery);
    if(!$GetPageNum) {
        echo "Error select: " . $conn->error . "<br/>";
    }
